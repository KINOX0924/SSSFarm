...
void checkAndReconnectWiFi(); // <<< 추가: WiFi 재연결 함수 프로토타입
...
void loop() {
  checkAndReconnectWiFi(); // <<< 추가: 루프 시작 시 항상 WiFi 연결 상태 확인 및 재연결
  ...
  if (currentMode == MANUAL && (currentTime - lastManualCommandTime >= manualModeTimeout)) {
    ...
    lcd.setCursor(0, 1);
    lcd.print("Mode: AUTO  "); // LCD 업데이트
  }

  // 로컬 센서 체크 및 자동 제어 로직 (AUTO 모드에서만 실행)
  ...
  // 배수 로직은 모드와 관계없이 자동 로직과 연계되어 있으므로 AUTO일 때만 실행
  if (currentMode == AUTO) {
    manageDrainage();
  }
  ...
  if (currentTime - lastServerSendTime >= serverSendInterval) {
    Serial.printf("\n>>> Attempting to send data. Time since last send: %lu ms\n", currentTime - lastServerSendTime);
    lastServerSendTime = currentTime;
    sendDataToServer();
    Serial.println("<<< Data sending process finished.");
  }
...
void checkAndReconnectWiFi() {
  if (WiFi.status() != WL_CONNECTED) {
    ...
    int attempts = 0;
    while (WiFi.status() != WL_CONNECTED && attempts < 20) { // 약 10초간 재연결 시도
      delay(500);
      Serial.print(".");
      attempts++;
    }
...
void fetchAndProcessCommands() {
  ...
  HTTPClient http;
  http.setTimeout(10000); // <<< 추가: 10초 타임아웃 설정
  http.begin(serverCommandUrl);

  Serial.println("Checking for commands from server...");
  ...
  if (httpCode == HTTP_CODE_OK) {
    ...
    // 유효한 명령 수신 시 수동 모드로 전환하고 타이머 리셋
    currentMode = MANUAL;
    ...
    if (httpCode > 0 && httpCode != 204) { // 204 No Content는 정상 응답이므로 에러로 처리 안함
      Serial.printf("[HTTP] GET... failed, error: %s\n", http.errorToString(httpCode).c_str());
    } else if (httpCode < 0) { // 타임아웃 등 라이브러리/연결 자체의 에러
...
void applyManualCommands(const JsonDocument& doc) {
  ...
  if (doc.containsKey("target_pump_state_1")) {
    ...
    if (state == "ON") digitalWrite(PUMP_PIN_1, HIGH);
    else if (state == "OFF") digitalWrite(PUMP_PIN_1, LOW);
  }
  if (doc.containsKey("target_pump_state_2")) {
    String state = doc["target_pump_state_2"];
    if (state == "ON") digitalWrite(PUMP_PIN_2, HIGH);
    else if (state == "OFF") digitalWrite(PUMP_PIN_2, LOW);
  }
...
void sendDataToServer() {
  ...
  JsonDocument doc;
  doc["temperature"] = (int)(temperature + 0.5); doc["humidity"] = (int)(humidity + 0.5);
  doc["soil_moisture_1"] = soil_moisture_1; doc["soil_moisture_2"] = soil_moisture_2;
  doc["light_level"] = (int)(light_level + 0.5); doc["water_level"] = water_level_percent;
  ...
  Serial.printf("POST request sent. Duration: %lu ms\n", httpEndTime - httpStartTime); // 소요 시간 출력
